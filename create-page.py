...
def create_new_api_gateway(rest_api_name):
    cmd = "aws apigateway create-rest-api --name " + rest_api_name + " --region eu-west-2"
...
def create_lambda(lambda_name):
    ...
    cmd = "aws lambda create-function --function-name " + lambda_name + " --runtime nodejs8.10 --role arn:aws:iam::081744355171:role/service-role/DefaultRole --handler index.handler --zip-file fileb://" + archive_zip_file
...
# (name) is used for both the api-gateway and the lambda.
def create_lambda_with_gateway(name):
    api_id = create_new_api_gateway(name)
    resource_id = create_api_resource(api_id, name)
    assign_get_method_to_resource(api_id, resource_id)
    assign_get_method_response(api_id, resource_id)
    lambda_arn = create_lambda(name)
    ...
    api_arn = get_api_arn(api_id, lambda_arn, name)
    add_lambda_execution_permissions(name, api_arn)
    lambda_invocation_url = get_lambda_invocation_url(api_id, name)
    folder_name = name
...
create_lambda_with_gateway(page_name + "-webpage")
